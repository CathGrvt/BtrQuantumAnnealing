...
def generate_hamiltonian(event: em.event, params: dict):
    ...
    dp = params.get('dp', 0.0)  # add default value of 0.0 for 'dp'
    ...
    A_dp = np.eye(N) * (-dp)  # added DP term
    b = np.ones(N) * delta
    ...
    A = -1 * (A_ang + A_bif + A_reg + A_gamma + A_dp)  # added DP term
    components = {
        'A_ang': -A_ang,
        'A_bif': -A_bif,
        'A_reg': -A_reg,
        'A_gamma': -A_gamma,
        'A_dp': -A_dp  # added DP term
    }
